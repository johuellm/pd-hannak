...
for hotel in stores:
    for experiment in experiments.iterkeys():        
        ...
        for test in experiments[experiment]:
            print os.path.join(procdir, 'hotel_' + hotel + '_' + experiment + '_' + test + '.txt')
            outfiles[test] = open(os.path.join(procdir, 'hotel_' + hotel + '_' + experiment + '_' + test + '.txt'), 'w')
        day = 0
        ...
        for folder in folder_list:
            ...
            for key in ht.keywords:
                try:
                    cpages[key] = ht.parse(hotel, os.path.join(datadir, hotel, experiment, folder, ht.control, key + '.html'))
                except:
            ...
            for test in experiments[experiment]:
                ...
                for key in ht.keywords:
                    ...
                    try:
                        tpage = ht.parse(hotel, os.path.join(datadir, hotel, experiment, folder, test, key + '.html'))
                    except:
...
for hotel in stores:
    for experiment in experiments.iterkeys():
        for target in experiments[experiment]:
            try:
                f = open(os.path.join(procdir, 'hotel_' + hotel + '_' + experiment + '_' + target + '.txt'))
            except:
                pass
            out = open(os.path.join(plotdir, 'hotel_' + hotel + '_' + experiment + '_' + target + '.txt'), 'w')
