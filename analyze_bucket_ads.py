...
spend_dir = 'spender'
hotels_spend_controls = ('spender', 'control')
hotels_spend_experiments = {
...
for hotel in stores:
    ...
    for test in experiments.iterkeys():
        outfiles[test] = open(os.path.join(procdir, 'hotel_ads_' + hotel + '_' + test + '.txt'), 'w')
    day = 0
    ...
    for folder in os.listdir(os.path.join(datadir, hotel, root_dir)):
        ...
        for key in ht.keywords:
            try:
                cpages[key] = ht.parse_ads(hotel, os.path.join(datadir, hotel, cexp, folder, ctest, key + '.html'))
            except:
        ...
        for test, pair in experiments.iteritems():
            ...
            for key in ht.keywords:
                ...
                try:
                    tpage = ht.parse_ads(hotel, os.path.join(datadir, hotel, cexp, folder, ctest, key + '.html'))
                except:
...
for hotel in stores:
    ...
    for target in experiments.iterkeys():
        f = open(os.path.join(procdir, 'hotel_ads_' + hotel + '_' + target + '.txt'))
        out = open(os.path.join(plotdir, 'hotel_ads_' + hotel + '_' + target + '.txt'), 'w')
        for line in f:
            e = line.split()
