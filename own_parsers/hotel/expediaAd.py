...
class ExpediaAdParser(HotelParser):

    def __init__(self, html_file):
        super(ExpediaAdParser, self).__init__(html_file)
        self.hotel_name_selector = '.hotelWrapper .segment.hotel.travelAd .details.showMile .infoCol .hotelName.fakeLink .hotelName strong'
        self.price_selector = '.hotelWrapper .segment.hotel.travelAd .details.showMile .priceCol .price'
    ...
    def parse_date(self):
        ...
        if date == '01/21/2014-01/22/2014':
            return 'A'
        elif date == '12/27/2013-01/04/2014' or date == '12/27/13-1/4/14' or date == '12/27/2013-1/4/2014':
            return 'B'
    ...
    def parse_prices(self):
        ...
        for price in prices:
            if len(price) == 1:
                my_price = price[0].text_content().strip().replace('$', '')
            else:
                my_price = price[1].text_content().strip().replace('$', '')
            acc.append(my_price)
        return acc
