...
class TravelocityParser(CarParser):
	...
	def parse_city(self):
		...
		my_city = my_city_list[0].text_content().strip(',').split()[0]
	...
	# gets the car types for travelocity
	def get_travelocity_car_types(self,car_type_list):
		acc = []
		for car in car_type_list:
			acc.append(car.text_content())
		return acc

	# gets the prices for travelocity
	def get_travelocity_prices(self, price_list):
		prices = []
	...
	# gets the agencies for travelocity
	def get_travelocity_agencies(self, agency_list):
		acc = []
	...
	def put_travelocity_together(self, price_list, car_type_list, agency_list):
		parsed = ''
		i = 0
		# this loop adds the cars from the table
		while i < len(price_list):
			price = price_list[i]
                        if 'No Result' in price:
                                i+=1
                                continue
	...
	def parse_table(self):
		...
		agency_list = self.get_travelocity_agencies(agency_list)
		car_type_list = self.get_travelocity_car_types(car_type_list)
		price_list = self.get_travelocity_prices(price_list)
		return self.put_travelocity_together(price_list, car_type_list, agency_list)
